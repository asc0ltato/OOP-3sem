...
public class Container
{
    ...
    public void Remove(object ob)
    {
        ...
        {
            throw new ContainerException("Контейнер пуст, нельзя удалить элемент.");
        }
...
public class HelloWorld
{
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            container1.Remove(exam);*/
            /* Debug.Assert(container1 == null, "Контейнер не должен быть пустым");*/
            Test test2 = new Test();
            test2.addQuestion("Ты учишься в бгту?", "Да");
            test2.addQuestion("Тебе нравится?", "Нет");
        ...
        {
            Console.WriteLine($"Ошибка ContainerException: {ex.Message}");
            Console.WriteLine($"Место исключения: {ex.StackTrace}");
        }
        catch (ChallengeException ex)
        {
            Console.WriteLine($"Ошибка ChallengeException: {ex.Message}");
            Console.WriteLine($"Место исключения: {ex.StackTrace}");
        }
        catch (TestException ex)
        {
            ...
            try
            {
                Console.WriteLine($"Ошибка TestException: {ex.Message}");
                Console.WriteLine($"Место исключения: {ex.StackTrace}");
                Console.WriteLine($"Причина исключения: {ex.Value}");
            ...
            {
                Console.WriteLine($"Внутренняя ошибка TestException: {e.Message}");
                Console.WriteLine($"Место исключения: {e.StackTrace}");
            }
        }
        catch
        {
